...
UpdateBansawCookie( target ) {
	decl String:timestamp[64];
	Format( timestamp, sizeof timestamp, "%d", c_bansaw[target] );
	SetClientCookie( target, cookie_bansaw, timestamp );
}

public Action:Command_Bansaw( client, args ) {
	if( args < 2 ) {
		ReplyToCommand( client, "[SM] Usage: sm_bansaw <player> <hours>" );
		return Plugin_Handled;
	}
	...
	new hours = StringToInt( arg );
	
	BaseComm_SetClientGag( target, true );
	c_bansaw[target] = GetTime() + hours * 60 * 60;
		
	UpdateBansawCookie(target);
	
	ReplyToCommand( client, "[SM] Bansaw activated on %N for %d hours. Use ungag to destroy the spell.", target, hours );
	return Plugin_Handled;
}
