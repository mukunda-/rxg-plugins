...
public Action:Command_SmHsay(client, args)
{
	...
	if( use_irc ) IRC_MsgFlaggedChannels( "relay", "\x031,15(ADMIN-HINT) %N: %s", client, text );
...
SendChatToAll(client, String:message[])
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		PrintToChat(i, "\x01 \x06(ALL) %s\x06: %s", nameBuf, message);
		
	}
	if( use_irc ) IRC_MsgFlaggedChannels( "relay", "\x031,9(ALL) %N: %s", client, message );
}
...
DisplayCenterTextToAll(client, String:message[])
{
	...
	if( use_irc ) IRC_MsgFlaggedChannels( "relay", "\x030,4(ALERT) %N: %s", client, message );
}
...
SendChatToAdmins(from, String:message[])
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			PrintToChat(i, "\x01 \x06(%sADMINS) %N: \x01%s", fromAdmin ? "" : "TO ", from, message);
		}	
	}
	
	if(use_irc ) {
		if( !fromAdmin ) {
			IRC_MsgFlaggedChannels( "relay", "\x034,8(TO ADMINS) %N: %s", from, message );
		} else {
			IRC_MsgFlaggedChannels( "relay", "\x031,11(ADMINS) %N: %s", from, message );
		}
	}
}
...
SendPanelToAll(from, String:message[])
{
	...
	if( use_irc ) IRC_MsgFlaggedChannels( "relay", "(PANEL) %N: %s", from, message );
